...
(async () => {
  ...
  // Click the NCR action button (not the tile)
  const ncrBtns = await frame.evaluate(() =>
    [...document.querySelectorAll('button')].filter(b => /NCR/i.test(b.innerText) && !/→/.test(b.innerText)).map(b => ({ text: b.innerText.trim(), onclick: b.getAttribute('onclick'), cls: b.className.slice(0,60) }))
  );
  console.log('NCR buttons:', JSON.stringify(ncrBtns, null, 2));

  // Try clicking it
  const ncrBtn = frame.locator('button').filter({ hasText: /^[⚠️\s]*NCR$/i }).first();
  if (await ncrBtn.count() > 0) {
    await ncrBtn.click();
    await page.waitForTimeout(5000);
    frame = await getAppFrame(page);
    const text = await frame.evaluate(() => document.body.innerText.slice(0, 200));
    console.log('\nAfter NCR click:\n', text);
    await page.screenshot({ path: '_e2e_ncr2.png', fullPage: true });
  } else {
    console.log('NCR button not found with strict match');
  }
