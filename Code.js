...
function doGet(e) {
  ...
  if (diag === 'perfinit') {
    var pi; try { pi = perfFormInit(); } catch(er){ pi='ERR '+er.message; }
    return ContentService.createTextOutput(String(pi)).setMimeType(ContentService.MimeType.TEXT);
  }
  if (diag === 'perfcachesize') {
  ...
  if (diag === 'notifypreview') {
    ...
    return ContentService.createTextOutput(String(npv)).setMimeType(ContentService.MimeType.TEXT);
  }
  if (diag === 'iqcpdf') {
    var ipc;
  ...
  if (diag === 'defercheck') {
    ...
    return ContentService.createTextOutput(String(dfc)).setMimeType(ContentService.MimeType.TEXT);
  }
  if (diag === 'savewrite') {
    var swp; try { swp = perfSaveWritePhase(); } catch(er){ swp = 'ERR ' + er.message; }
    return ContentService.createTextOutput(String(swp)).setMimeType(ContentService.MimeType.TEXT);
  }
  if (diag === 'savebreak') {
    var sbd;
    try { sbd = perfSaveBreakdown(e.parameter.doc || ''); } catch(er){ sbd = 'ERR ' + er.message; }
    return ContentService.createTextOutput(String(sbd)).setMimeType(ContentService.MimeType.TEXT);
  }
  if (diag === 'sharemsg') {
  ...
  if (diag === 'zeroqtyfix') {
    var zqf;
    try { zqf = zeroQtyBackfill(String(e.parameter.confirm || '') === 'YES'); }
    catch(er){ zqf = 'ERR ' + er.message; }
    return ContentService.createTextOutput(String(zqf)).setMimeType(ContentService.MimeType.TEXT);
  }
  ...
  if (diag === 'imgprobe') {
    ...
    catch(er){ ipr = 'ERR ' + er.message; }
    return ContentService.createTextOutput(String(ipr)).setMimeType(ContentService.MimeType.TEXT);
  }
  if (diag === 'pdfshow') {
    var pds;
...
function getFormHtml(type) {
  ...
  var cacheKey = 'pmqms_formhtml_v183_' + String(type || 'Landing');
  try {
